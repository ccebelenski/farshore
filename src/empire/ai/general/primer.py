"""The general's rules primer + STANDING DOCTRINE, as a module constant.

Ported VERBATIM from `lab/prompts/stability/primer_d.txt` — the pinned prompt
that passed integration handshake (b). The primer is the static cache-prefix
of every epoch (planning/08 wrinkle #5: cache-native layout), so its text must
stay byte-stable; edit it only alongside a lab re-run, and keep this constant
and the lab file in lockstep.
"""
...
STANDING DOCTRINE  (tested — lessons your service has already paid for)
  ...
  - Meet surprises with uncommitted reserves first, at the point of
    threat — not at the nearest garrison. A committed task force pulled
    off its mission costs you two missions.
